    ...
    if results_pos != -1:
        ...
        if array_start != -1:
            ...
            while pos < len(text):
                ...
                pos = obj_end + 1
            if recovered:
                logger.warning("잘린 JSON 부분 복구: %d건", len(recovered))
...
class GeminiSearchProvider(BaseProvider):
    ...
    def search(self, claim_text: str, cutoff_date: str, limit: int = 10) -> list[SearchResult]:
        if self.patent_number:
            logger.info("Gemini 검색: 특허번호 %s 기반", self.patent_number)
            ...
            logger.info("Gemini 검색: 청구항 텍스트 기반 (%d자)", len(claim_text))
        ...
        logger.info("Gemini Google Patents 웹 검색 중 (최대 5분)...")
        t0 = time.time()
        try:
            response = self.router.call(prompt, timeout=300)
        except Exception as e:
            logger.error("Gemini 검색 LLM 호출 오류 (%.0f초 후): %s", time.time() - t0, e)
            return []
        logger.info("Gemini 응답 수신: %.0f초, %d자", time.time() - t0, len(response))
    ...
    def _parse(self, response: str) -> list[SearchResult]:
        try:
            ...
            logger.error("Gemini 검색 JSON 파싱 실패: %s", e)
            logger.debug("응답 앞부분: %s", response[:300])
            return []
        ...
        logger.info("Gemini 검색 결과: %d건", len(results))
        return results
